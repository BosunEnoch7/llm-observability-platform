...
- Full Docker Compose evidence: pending Docker Desktop access.
...
- Portfolio evidence checklist and API-only evidence files.
...
- Docker Compose config validated when Docker config was accessible.
...
The remaining work is environment execution, not repository implementation.
Before running real Azure deployment, confirm subscription ownership, budget,
provider availability, RBAC, and teardown approval.
